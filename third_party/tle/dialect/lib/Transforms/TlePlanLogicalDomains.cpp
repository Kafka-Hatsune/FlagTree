...
// site. Every logical stage is fully initialized. Tensor padding along either
// capacity or payload is never stored, and consumes no shared memory.
static void applyLogicalInitializer(LogicalRootRewriteAction &action,
  ...
  // The frontend uses a leading-axis broadcast to satisfy local_alloc's
  // equal-shape verifier. Share its original register payload across slots
  // instead of distributing/extracting a replicated rank-3 tensor.
