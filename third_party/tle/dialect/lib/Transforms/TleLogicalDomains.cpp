  ...
  static LogicalDomainFact join(const LogicalDomainFact &lhs,
    ...
    if (lhs.kind == Kind::MemDesc) {
      ...
      if (a.physicalShape != b.physicalShape ||
          ...
          a.isStage != b.isStage || a.transposed != b.transposed ||
  ...
  bool operator==(const LogicalDomainFact &other) const {
    ...
    if (kind == Kind::MemDesc)
      return memdesc.physicalShape == other.memdesc.physicalShape &&
             ...
             memdesc.transposed == other.memdesc.transposed &&
...
LogicalDomainContext::processCandidateAlloc(Operation *operation,
  ...
  // Upstream local_alloc requires identical initializer/carrier shapes.
  // Register tensors pad capacity as well as the payload fragment. This
  // leading padding is transient: the storage plan keeps logical[0] slots.
  if (alloc.getSrc())
    expected[0] = nextPowerOfTwo(capacity);
  ...
  if (Value initializer = alloc.getSrc()) {
    ...
    if (const auto *fragment = lookupTensor(initializer)) {
      LogicalShape initializerDomain(logical.begin(), logical.end());
      initializerDomain[0] = expected[0];
      if (ArrayRef<int64_t>(materializeLogicalShape(initializer, *fragment)) !=
          ArrayRef<int64_t>(initializerDomain))
  ...
  for (auto [index, operand] : llvm::enumerate(operation->getOperands())) {
    ...
    if (blockShape.size() < 2 ||
      ...
      return emitError(copy, index,
                       ...
                       "leading dimensions");
    SmallVector<int64_t, 2> storageTile(blockShape.take_back(2));
    auto carrier =
        dyn_cast<ttg::NVMMASharedEncodingAttr>(dstType.getEncoding());
    unsigned fragmentAxis =
        !llvm::isPowerOf2_64(state->logicalShape[0]) ? 0 : 1;
    if (!carrier || storageTile[fragmentAxis] < kExactSMEMFragmentQuantum ||
        llvm::any_of(storageTile,
                     [](int64_t extent) {
                       return extent <= 0 || !llvm::isPowerOf2_64(extent);
                     }) ||
        !isLegalSMEMTile(state->logicalShape, storageTile,
                         getExactSMEMStorageTileEncoding(
                             carrier, storageTile, dstType.getElementType())))
      return emitError(
          copy, index,
          "logical TMA descriptor block must be a power-of-two divisor "
          "containing a complete shared core and at least 16 fragment "
          "elements");
    auto logical =
    ...
    if (phase == LogicalDomainPhase::Plan) {
      ...
      if (root->fixedStorageTile &&
          failed(selectRootStorageTile(*root, storageTile, copy)))
        return failure();
      root->storageTileShape = storageTile;
      root->fixedStorageTile = true;
...
LogicalDomainContext::processLogicalPointerCopy(Operation *operation,
  ...
  if (!root->fixedStorageTile && root->storageTileShape.empty())
...
static LogicalResult selectRootSMEMPlan(LogicalRootRewriteAction &root,
  ...
  plan.module.walk([&](WGMMAOp dot) {
    ...
    consumers.push_back(
        {state.transposed, static_cast<unsigned>(*instructionK),
  ...
  if (root.initializer && preferredTile.empty()) {
  ...
  for (int64_t rows = 8; rows <= shape[0]; rows *= 2) {
    for (int64_t cols = 8; cols <= shape[1]; cols *= 2) {
      ...
      if (shape[0] % rows || shape[1] % cols ||
          ...
          (root.fixedStorageTile && tile != preferredTile))
        continue;
      for (bool transposed : {false, true}) {
        for (unsigned swizzle : {128u, 64u, 32u, 0u}) {
          auto encoding = ttg::NVMMASharedEncodingAttr::get(
              root.alloc.getContext(), swizzle, transposed, bits, false,
              carrier.getCTALayout());
          if (!isLegalSMEMTile(shape, tile, encoding))
            continue;
          // TMA's tensor descriptor fixes its box and its upstream shared
          // swizzle. Pointer producers sharing that root adapt to this choice.
          if (root.fixedStorageTile &&
              encoding !=
                  getExactSMEMStorageTileEncoding(carrier, tile, elementType))
            continue;
          ...
          for (const Consumer &consumer : consumers) {
            ...
            if (!supportsWGMMAOperandTranspose(elementType) &&
                transposed == consumer.transposed) {
              legal = false;
              break;
            }
            auto operand = planWGMMAOperand(storage, consumer.transposed,
          ...
          for (auto &copy : root.pointerCopies) {
            unsigned vectorBytes = copyVectorBytes(copy, transposed ? 0 : 1);
          ...
          if (root.initializer) {
            ...
            int64_t capacity = root.logicalShape[0];
            transactions += capacity * storage.stageBytes() / 16;
            waves += capacity * shape[0] * shape[1] / (micro[0] * micro[1]);
          }
          int64_t tiles = shape[0] * shape[1] / (rows * cols);
          waves += root.copies.size() * tiles;
          Cost cost{instructions,
                    ...
                    tiles,
                    tile == preferredTile &&
                            transposed == carrier.getTransposed()
                        ? 0
                        : 1};
