...
public:
  SMEMLayoutPlan(ArrayRef<int64_t> shape, ArrayRef<int64_t> tile,
      ...
      : rows(shape[0]), cols(shape[1]), tileRows(tile[0]), tileCols(tile[1]),
        encoding(encoding),
        tileInv(gpu::nvmmaSharedToLinearLayout(tile, encoding).pseudoinvert()),
        plainTileInv(gpu::nvmmaSharedToLinearLayout(tile, encoding, true)
                         .pseudoinvert()) {}

  int64_t rows, cols, tileRows, tileCols;
  gpu::NVMMASharedEncodingAttr encoding;
  LinearLayout tileInv, plainTileInv;
  ...
  int64_t offset(int64_t row, int64_t col, bool plain = false) const {
    const auto &ll = plain ? plainTileInv : tileInv;
    auto dims = llvm::to_vector(ll.getInDimNames());
    auto local =
        ll.apply({{dims[0], row % tileRows}, {dims[1], col % tileCols}})[0]
            .second;
...
struct WGMMAOperandPlan {
  ...
  bool transposed() const {
    return storage.encoding.getTransposed() != viewTransposed;
  }
  DictionaryAttr getAttr() const {
...
planWGMMAOperand(const SMEMLayoutPlan &storage, bool viewTransposed,
  ...
  bool transposed = storage.encoding.getTransposed() != viewTransposed;
  ...
  int64_t coreK = transposed ? span : 8;
  int64_t coreN = transposed ? 8 : span;
  auto address = [&](int64_t k, int64_t n) {
    return viewTransposed ? storage.offset(n, k, true)
                          : storage.offset(k, n, true);
  };
  int64_t strideK = address(coreK, 0);
  int64_t strideN = address(0, coreN);
  int64_t lbo = transposed ? strideK : strideN;
  int64_t sbo = transposed ? strideN : strideK;
  if (!storage.encoding.getSwizzlingByteWidth() && !transposed)
    std::swap(lbo, sbo);
  ...
  if (transposed && storage.encoding.getSwizzlingByteWidth() &&
      instructionK <= coreK)
    lbo = 16;
