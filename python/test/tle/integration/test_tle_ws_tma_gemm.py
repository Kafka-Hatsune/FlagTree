...
@triton.jit
def ws_tma_multi_slot_gemm_warp_specialize_kernel(
    a_desc,
    b_desc,
    c_ptr,
    stride_cm: tl.constexpr,
...
def ws_tma_multi_slot_gemm_warp_specialize(A, B, C, launch_num_warps, block_k, num_slots):
    assert A.ndim == 2 and B.ndim == 2 and C.ndim == 2
    assert A.shape[1] == B.shape[0]
    assert C.shape == (A.shape[0], B.shape[1])
    assert A.dtype == torch.float16 and B.dtype == torch.float16 and C.dtype == torch.float32
    ...
    a_desc = TensorDescriptor.from_tensor(A, block_shape=[block_m, block_k])
    b_desc = TensorDescriptor.from_tensor(B, block_shape=[block_k, block_n])
    return ws_tma_multi_slot_gemm_warp_specialize_kernel[(1, )](
        a_desc,
        b_desc,
        C,
        C.stride(0),
...
class TestTLEAsyncTasksTmaGemm:
    ...
    @pytest.mark.parametrize("launch_num_warps", [4])
    def test_warp_specialize_multi_slot_producer_consumer_wgmma(self, launch_num_warps):
        torch.manual_seed(3026 + launch_num_warps)
        block_m, block_n, block_k = 64, 16, 16
        k_tiles, num_slots = 4, 2
        ...
        kernel = ws_tma_multi_slot_gemm_warp_specialize(a, b, c, launch_num_warps, block_k, num_slots)
        assert "tt.call" not in kernel.asm["ttgir"]
